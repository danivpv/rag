...
| **LLM** | Google Gemini | Amazon Bedrock Claude 3 Haiku ($0.25/M input tokens) |
...
### Streamlit version (`requirements.txt`)
```
streamlit>=1.28.0
langchain>=0.1.0
...
**For our AWS version we will replace:**
- `langchain-google-genai` → `langchain-aws` or `boto3` direct Bedrock calls
- `google-generativeai` → `boto3` Bedrock runtime
- Keep: `langchain`, `langchain-community`, `faiss-cpu`, `pypdf`, rest
